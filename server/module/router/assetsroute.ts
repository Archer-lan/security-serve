...
class AssetsRoute extends BaseRoute{
    ...
    async Upload(){
        try{
            ...
            // let json = JSON.parse(this.model.info);
            let res=await this.assetsService.Upload(this.request,this.model.file,this.model.proid);
            // console.log(json);
            return {success:Responesecode.SUCCESS};
        }catch (e) {
            return {success:Responesecode.FAIL,msg:e}
        }
    }
}